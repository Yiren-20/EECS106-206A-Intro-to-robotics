...
#     # x2 = target_position[0] - np.cos(yaw) * np.sqrt((target_position[0]-x1)**2 + (target_position[1]-y1)**2)
#     x2 = x1 + (target_position[0] * np.cos(yaw) - target_position[1] * np.sin(yaw))
#     # y2 = target_position[1] - np.sin(yaw) * np.sqrt((target_position[0]-x1)**2 + (target_position[1]-y1)**2)
...
def plan_curved_trajectory(target_position):
    ...
    x2 = x1 + dx * math.cos(yaw) - dy * math.sin(yaw)
    y2 = y1 + dx * math.sin(yaw) + dy * math.cos(yaw)

    node.get_logger().info(f'Start: ({x1:.2f}, {y1:.2f}, {math.degrees(yaw):.1f}) degrees')
    node.get_logger().info(f'Target: ({x2:.2f}, {y2:.2f}) in odom frame')
